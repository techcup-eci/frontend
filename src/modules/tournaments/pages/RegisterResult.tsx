...
export default function RegisterResult() {
  ...
  const addHomeGoal = () => {
    ...
    setHomeGoals([...homeGoals, { key, playerId: "", teamId: homeTeamId, minute: 0, goalType: "GOAL" }]);
  };

  const addAwayGoal = () => {
    const key = `ag-${Date.now()}`;
    setAwayGoals([...awayGoals, { key, playerId: "", teamId: awayTeamId, minute: 0, goalType: "GOAL" }]);
  };

  const addHomeCard = () => {
    const key = `hc-${Date.now()}`;
    setHomeCards([...homeCards, { key, playerId: "", teamId: homeTeamId, type: "YELLOW", minute: 0 }]);
  };

  const addAwayCard = () => {
    const key = `ac-${Date.now()}`;
    setAwayCards([...awayCards, { key, playerId: "", teamId: awayTeamId, type: "YELLOW", minute: 0 }]);
  };

  const updateHomeGoal = (key: string, field: keyof PlayerGoal, value: string | number) => {
    setHomeGoals(homeGoals.map((g) => (g.key === key ? { ...g, [field]: value } : g)));
  };

  const updateAwayGoal = (key: string, field: keyof PlayerGoal, value: string | number) => {
    setAwayGoals(awayGoals.map((g) => (g.key === key ? { ...g, [field]: value } : g)));
  };

  const updateHomeCard = (key: string, field: keyof PlayerCard, value: string | number) => {
    setHomeCards(homeCards.map((c) => (c.key === key ? { ...c, [field]: value } : c)));
  };

  const updateAwayCard = (key: string, field: keyof PlayerCard, value: string | number) => {
    setAwayCards(awayCards.map((c) => (c.key === key ? { ...c, [field]: value } : c)));
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const totalHomeGoals = homeGoals.reduce((sum, g) => sum + (Number(g.minute) > 0 ? 1 : 0), 0);
    const totalAwayGoals = awayGoals.reduce((sum, g) => sum + (Number(g.minute) > 0 ? 1 : 0), 0);
